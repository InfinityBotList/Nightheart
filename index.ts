...
paths.forEach((url) => {
    app.get<{
        ...
        // Fetch bot
        let spider = await fetch(
            `https://spider.infinitybots.gg/bots/${req.params.id}`
        );
        ...
        let icon = await loadImage(
            "https://cdn.infinitybots.xyz/images/core/InfinityNewTrans.png"
        );
        ...
        switch (req.query.size) {
            case "large":
                ...
                image = new Canvas(400, 240)
                    ...
                    .printText(
                        `${
                            bot.shards === 0 ? "N/A" : formatNumbers(bot.shards)
                        } SHARDS`,
                        217,
                        197
                    )
                    ...
                    .printText(
                        `${
                            bot.votes === 0 ? 0 : formatNumbers(bot.votes)
                        } VOTES`,
                        217,
                        167
                    )
                    ...
                    .setTextSize(20)
                    .printText(
                        bot.user.username.length > 25
                            ? bot.user.username.slice(0, 25) + "..."
                            : bot.user.username,
                        40,
                        25
                    )
...
app.setErrorHandler(function (error, request, reply) {
    if (error instanceof fastify.errorCodes.FST_ERR_BAD_STATUS_CODE) {
        ...
        reply
            ...
            .send({
                message:
                    "Something broke: Invalid status code was attempted to be sent",
                error: false,
            });
    } else {
